  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    ...
    <div class="space-y-6">
      ...
      <div class="bg-white border border-slate-200 rounded-xl shadow-sm overflow-hidden">
        <div class="px-6 py-4 border-b border-slate-100 bg-slate-50/50">
          <h2 class="font-bold text-slate-900" style="font-family:Manrope">
            <span class="material-symbols-outlined text-base align-text-bottom mr-1">receipt_long</span>
            Proof of Sale <span class="text-slate-400 font-normal text-sm">(<?= count($proofPaths) ?> file<?= count($proofPaths) > 1 ? 's' : '' ?>)</span>
          </h2>
        </div>
        <div class="p-6">
          <?php if ($isAdmin): ?>
          <div class="flex flex-wrap gap-3">
            ...
            <a href="/transactions/<?= $txn->id ?>/proof?index=<?= $idx ?>" target="_blank"
              ...
              Proof #<?= $idx + 1 ?>
              <span class="text-slate-400 font-normal text-[10px]"><?= htmlspecialchars(strtoupper(pathinfo($path, PATHINFO_EXTENSION))) ?></span>
            </a>
            <?php endforeach; ?>
          </div>
          ...
          <p class="text-[10px] text-slate-400 mt-2">Only Managers &amp; Admins can view these documents.</p>
          <?php endif; ?>
        </div>
      </div>
      <?php endif; ?>
