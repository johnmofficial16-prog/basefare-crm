 ...
 * Shown to users with role = 'manager'.
 * Has attendance board, shift scheduling, team-facing CRM links, users (read-only), e-tickets.
 * Does NOT expose: Settings, Error Console, Activity Log, Payroll, Vouchers.
 *
...
<aside class="fixed left-0 top-0 h-full w-60 bg-white border-r border-gray-100 flex flex-col z-30 shadow-sm">
  ...
  <div class="px-6 py-5 border-b border-gray-100">
    <a href="/dashboard" class="flex items-center gap-2 no-underline">
      ...
      <span class="font-headline font-extrabold text-primary text-sm leading-tight">Base Fare<br><span class="text-on-surface-variant font-medium text-xs">Manager</span></span>
    </a>
  </div>
  ...
  <div class="px-5 py-4 border-t border-gray-100">
    <div class="flex items-center gap-3 mb-3">
      ...
      <div class="flex-1 min-w-0">
        <p class="text-xs font-bold text-on-surface truncate"><?= htmlspecialchars($_SESSION['user_name'] ?? 'Manager') ?></p>
        <p class="text-[10px] text-on-surface-variant capitalize">Manager</p>
      </div>
    </div>
