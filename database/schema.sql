...
CREATE TABLE IF NOT EXISTS `transactions` (
  ...
  `dispute_status`          ENUM('dispute_opened','chargeback_received','refunded_dispute','resolved') NULL DEFAULT NULL,
  `dispute_notes`           TEXT NULL,
  `dispute_flagged_at`      DATETIME NULL,
  `dispute_flagged_by`      INT NULL,
  `gateway_status`          ENUM('charge_successful','charge_declined') NULL DEFAULT NULL,
  `gateway_transaction_id`  VARCHAR(100) NULL,
  `gateway_actioned_at`     DATETIME NULL,
  `gateway_actioned_by`     INT NULL,
  `created_at`              DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  `updated_at`              DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`),
